...
serverT <- function(input, output, session){
  ...
  outputOptions(output, "auto", suspendWhenHidden=FALSE)
  # Answer array values
  ...
  observeEvent(input$select,{
    ...
    if(!answered()){
      ...
      answerString <- AnswerCheckShiny(answerArray$correct, answerArray$index,
                                       as.numeric(input$answerSelect))$string
      id <- showNotification(renderText(answerString))
    }
    })
    ...
    if(answered()){
      if(switch(difficulty$Concept,
                    ...
                    stage() < 4)){
                      ...
                      updateRadioButtons(session, "difficultyUpdateConcept",
                                         label = stringConcept)
                      updateRadioButtons(session, "difficultyUpdateCalculation",
...
uiPlot <- fluidPage(
  ...
  plotOutput("plot"),
  conditionalPanel(condition = "output.q",
                  sliderInput("slider", "Slide to change line", value = 0, min = -3, max = 3, step=0.01),
                  actionButton("select", "Select Answer"),
                  textOutput("selected")),
  conditionalPanel(condition = "!output.q",
                  radioButtons("critical", label = "Is this value above 0.5?",
                               choiceNames = c("Yes", "No"), choiceValues = 1:2),
                  actionButton("evaluate", "Select answer"),
                  textOutput("result"))
)
...
serverPlot <- function(input, output){
  ...
  slider <- reactive(input$slider)
  select <- reactiveVal(0)
  answer <- reactive("")
  
  question <- reactiveVal(FALSE)
  output$q <- reactive(question())
